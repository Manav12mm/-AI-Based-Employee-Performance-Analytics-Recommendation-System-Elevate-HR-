...
const Login = () => {
  ...
  return (
    <div className="max-w-md mx-auto mt-10 bg-white p-8 border border-stone-200 rounded-xl shadow-sm">
      <h2 className="text-2xl font-bold text-center mb-6 text-stone-800">Login to ElevateHR</h2>
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <div>
          <label className="block text-stone-700 mb-1 text-sm font-medium">Password</label>
          <input 
            type="password" 
            className="w-full px-4 py-2 border border-stone-200 bg-stone-50 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 transition-shadow"
            value={password}
      ...
      <div className="mt-6 text-center text-sm text-stone-500">
        Don't have an account? <Link to="/register" className="text-amber-600 hover:text-amber-700 font-medium ml-1 hover:underline">Register here</Link>
      </div>
    </div>
  );
};
